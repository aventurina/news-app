...
export function createApp({ apiKey = process.env.NEWS_API_KEY, fetchImpl = fetch } = {}) {
    const app = express();
    app.use(cors());

    // Cache upstream responses server-side too, so multiple visitors querying
    // the same headlines don't each burn a separate call against NewsAPI's rate limit.
    const cache = new Map();
